...
# YOUTUBE 1: So first we need a function that generates the password using the key as the same length of the inputted message. Example with the key 'like' -> subscribe to my channel: likelikel ik el ikelike
def generate_key(message, key, alphabet='abcdefghijklmnopqrstuvwxyz'):
    ...
    # YOUTUBE 2-1: One problem i encountered was encrypting messages with space and other specail characters included. So this for loop stores the index of those characters in a list, (make a compared image of password and message with spaces included)
...
def encrypt(message_to_encrypt, key='password', alphabet='abcdefghijklmnopqrstuvwxyz'):
    ...
    # YOUTUBE 5: Here we call the generate_key function to create the password for the input. In this case the input is the message that the user wants to encrypt.
    generated_keys = generate_key(message_to_encrypt,key, alphabet)
    #print(f'Generate Key function call in encrypt: {generated_keys}')

    ciphered_alphabets = []

    # YOUTUBE 6-1: And in this for loop we create 'Caeser Ciphers' for every individual letter in the 'generated_key'. (Make an image animation for this part as well) --Creates the ciphered alphabet for each character in the generated keyword--
...
# YOUTUBE 7: And for decrypting we will use the indexes of the letters in the encrypted message from the ciphered alphabets as I explained. Lets create a function to take arguments for the message to decrypt, key and an alphabet. The last two will have default values.
def decrypt(message_to_decrypt, key='password', alphabet='abcdefghijklmnopqrstuvwxyz'):
    ...
    # YOUTUBE 10: And again we are searching the original English Alphabet with the indexes of the letters in the decrypted message to decrypt our message
